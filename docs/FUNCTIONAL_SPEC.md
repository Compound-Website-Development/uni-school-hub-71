...
- **22 September 2026 — Database integrity/security hardening:** revoked anonymous execution of internal class/admin helper RPCs (`can_manage_student`, `has_admin_permission`, `teaches_class`), added a unique `(student_id, date)` attendance index to prevent duplicate daily registers, and added `NOT VALID` CA 0–40 / Exam 0–60 checks so new grade writes cannot introduce out-of-range scores while preserving the three legacy rows that still need school confirmation.
...
- Last synchronisation: 2026-09-22T11:39:29.947Z
- Commit: `267d5542a3ea2237b66fbe021b37c0f887143d57`
- Trigger: push

- `M` `src/components/layout/StudentLayout.tsx`
...
| 3 | Resolved | CBT submission | `TakeExam` now calls the server-side `submit_exam` function, which scores against protected answer keys and records the result | Fixed in current implementation | Retain RPC tests when exam behavior changes |
| 4 | Resolved | Teacher/student RLS | Student access is now scoped through `teaches_class()` or direct ownership/parent linkage, while admins retain full school access | Fixed in current policies | Re-test policy coverage when adding a new academic table |
| 5 | Resolved | Finance permissions | Sensitive finance functions call `has_admin_permission()` before generating invoices, recording payments, or applying discounts | Fixed for finance workflows | Apply the same capability model to future privileged modules |
| 6 | Resolved | Authentication role assignment | The login screen no longer exposes self-registration, and `handle_new_user()` no longer trusts user-editable metadata for privileged roles | Fixed 23 August 2026 | Keep privileged account creation in trusted server flows only |
...
| 26 | Resolved | `src/hooks/useAuth.tsx` admin preview | Admins without a linked shadow record previously inherited the oldest real pupil as preview data | Unsafe fallback from an earlier demo implementation | Removed 23 August 2026; admins now see isolated presentation fallbacks until deliberately linked |
...
| CBT auto-scoring | Complete (server-side RPC) |
...
| Admin permission flags | Partially enforced (finance paths) |
