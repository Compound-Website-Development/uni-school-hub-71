...
export const CommunityWall = () => {
  ...
  const loadPosts = useCallback(async () => {
    ...
    setPosts(data ? await enrichPosts(data) : []);
    setLoading(false);
  }, [enrichPosts]);

  // Keep one live reference to the loader so the realtime channel is created
  // exactly once. Re-using a channel topic that is already subscribed throws
  // "cannot add postgres_changes callbacks after subscribe()".
  const loadRef = useRef(loadPosts);
  loadRef.current = loadPosts;
