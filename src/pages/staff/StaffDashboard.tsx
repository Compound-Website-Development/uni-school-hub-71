  ...
  <div className="dashboard-surface dashboard-staff space-y-5">
   <section className="portal-hero grid gap-6 md:grid-cols-[1fr_auto]">
    <div className="portal-hero-copy max-w-none">
      <p className="text-[10px] font-extrabold uppercase tracking-[.22em] text-[#2f8fca]">Staff workspace</p>
      <h1 className="portal-display mt-2 text-4xl font-extrabold leading-[1] text-[#17394c] md:text-6xl">Good day, {userName.split(" ")[0]}.</h1>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <Link to="/staff/attendance" className="rounded-full bg-[#2f8fca] px-4 py-2.5 text-xs font-extrabold text-white">Take attendance</Link>
        <Link to="/staff/gradebook" className="rounded-full border border-border bg-white px-4 py-2.5 text-xs font-extrabold text-[#1d668a]">Open gradebook</Link>
      </div>
    </div>
   ...
   <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
    <Card className="portal-surface overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between pb-3"><div><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#2f8fca]">Today</p><CardTitle className="portal-display mt-1 text-2xl font-extrabold">Teaching schedule</CardTitle></div><Clock3 className="h-5 w-5 text-[#7c877f]"/></CardHeader>
      <CardContent>
       {upcomingClasses.length ? <div className="space-y-2">{upcomingClasses.map((item,i)=><div key={`${item.time}-${i}`} className="flex items-center gap-3 rounded-2xl bg-[#f3f9fc] p-3.5"><div className="w-16 shrink-0 text-center"><p className="text-sm font-extrabold text-[#1c82b3]">{item.time}</p><p className="text-[9px] uppercase tracking-wider text-muted-foreground">start</p></div><div className="min-w-0 flex-1"><p className="text-sm font-extrabold text-[#274e63]">{item.className} · {item.subject}</p><p className="mt-0.5 text-xs text-muted-foreground">{item.room||"Classroom"} · timetable entry</p></div></div>)}</div> :
       <div className="rounded-2xl border border-dashed border-[#d7dfd9] p-6"><p className="font-bold text-[#36586b]">No timetable entries published today.</p><p className="mt-1 text-xs leading-5 text-muted-foreground">The schedule will appear here when the school adds it.</p></div>}
      </CardContent>
    </Card>
    ...
    <div className="space-y-4">
      ...
      <Card className="portal-surface">
       <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><AlertTriangle className="h-4 w-4 text-[#2385b2]"/>Needs your attention</CardTitle></CardHeader>
       <CardContent>{attention.length ? <div className="space-y-2">{attention.map(item=><Link key={item.kind} to={item.href} className="flex items-center gap-3 rounded-2xl bg-[#eef9ff] p-3.5 hover:bg-[#e1f5ff]"><div className="grid h-9 w-9 place-items-center rounded-xl bg-white text-[#2385b2]"><ArrowRight className="h-4 w-4"/></div><div className="min-w-0"><p className="text-sm font-bold">{item.label}</p><p className="text-[10px] text-muted-foreground">{item.kind}</p></div></Link>)}</div> : <p className="rounded-2xl bg-[#f3f9fc] p-4 text-sm text-muted-foreground">Nothing is waiting for you right now.</p>}</CardContent>
      </Card>
    </div>
   </section>

   <section className="portal-surface rounded-[26px] p-5 md:p-6">
    <div className="flex items-center justify-between"><div><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#2f8fca]">My work</p><h2 className="portal-display mt-1 text-2xl font-extrabold text-[#17394c]">Open a workspace</h2></div><TrendingUp className="h-5 w-5 text-[#7c877f]"/></div>
    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {workbench.map(item=><Link key={item.href} to={item.href} className="staff-dashboard-link group p-4"><div className={`staff-art staff-art-${item.kind}`} aria-hidden="true"><span/><i/><b/></div><p className="portal-display mt-3 text-lg font-extrabold">{item.label}</p><p className="mt-1 text-xs leading-5 text-muted-foreground">{item.note}</p><span className="mt-3 inline-flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-[.12em] text-[#2f8fca]">Open <ArrowRight className="h-3 w-3"/></span></Link>)}
    </div>
   </section>

   {performance && <section className="portal-surface rounded-[26px] p-5 md:p-6"><div className="flex items-center justify-between"><div><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[#2f8fca]">Your activity</p><h2 className="portal-display mt-1 text-2xl font-extrabold text-[#17394c]">Work completed</h2></div><BookOpen className="h-5 w-5 text-[#7c877f]"/></div><div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">{[{label:"Classes teaching",value:performance.classesTeaching},{label:"Attendance marked",value:performance.attendanceMarked},{label:"Grades entered",value:performance.gradesEntered},{label:"Lesson plans",value:performance.lessonPlansCreated}].map(item=><div key={item.label} className="rounded-2xl bg-[#f3f9fc] p-4"><p className="portal-display text-2xl font-extrabold text-[#17394c]">{item.value}</p><p className="mt-1 text-[11px] text-muted-foreground">{item.label}</p></div>)}</div></section>}
  </div>
 </StaffLayout>;
};
