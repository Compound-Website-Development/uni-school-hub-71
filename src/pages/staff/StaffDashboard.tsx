  ...
  <div className="dashboard-surface dashboard-staff space-y-5">
   ...
   <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
    <Card className="portal-surface overflow-hidden">
      ...
      <CardContent>
       {upcomingClasses.length ? <div className="space-y-2">{upcomingClasses.map((item,i)=><div key={`${item.time}-${i}`} className="flex items-center gap-3 rounded-2xl bg-[#f3f9fc] p-3.5"><div className="w-16 shrink-0 text-center"><p className="text-sm font-extrabold text-[#2578a7]">{item.time}</p><p className="text-[9px] uppercase tracking-wider text-muted-foreground">start</p></div><div className="min-w-0 flex-1"><p className="text-sm font-extrabold text-[#274e63]">{item.className} · {item.subject}</p><p className="mt-0.5 text-xs text-muted-foreground">{item.room||"Classroom"} · timetable entry</p></div></div>)}</div> :
