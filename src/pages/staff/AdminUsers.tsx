...
const AdminUsers = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch all teachers
      const { data: teacherRows } = await (supabase as any).rpc("staff_teacher_records");
      const teachersData = (teacherRows || [])
      ...
      if (adminData && teachersData) {
        const adminUsers: AdminUser[] = adminData.map((admin) => {
          ...
          return {
            ...
            can_manage_fees: admin.can_manage_fees,
              ...
              <div className="space-y-4">
                {admins.map((admin) => (
                  <div key={admin.id} className="border border-border rounded-lg p-4">
                    ...
                    {admin.can_manage_fees && !admin.can_add_admins && (
                      <Badge variant="outline" className="mb-3">Bursar</Badge>
                    )}

                    <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-muted-foreground">Manage Fees</span>
                        <Switch
                          checked={admin.can_manage_fees}
                          onCheckedChange={(v) => updatePermission(admin.id, "can_manage_fees", v)}
