...
const StaffMessages = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel("staff-messages")
  ...
  if (isLoading) {
    return <StaffLayout title="Messages"><div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div></StaffLayout>;
  }
    ...
    <StaffLayout title="Messages">
      <div className="space-y-6 animate-fade-in">
        <Card className="border-border/50">
          <CardHeader className="pb-2"><CardTitle className="text-base">Compose</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <Input placeholder="Subject" value={newSubject} onChange={(e) => setNewSubject(e.target.value)} />
            <Textarea placeholder="Message..." value={newBody} onChange={(e) => setNewBody(e.target.value)} rows={3} />
        ...
        <Card className="border-border/50">
          ...
          <CardContent>
            {messages.length === 0 ? (
              <div className="text-center py-8"><Inbox className="w-10 h-10 mx-auto mb-2 text-muted-foreground/30" /><p className="text-sm text-muted-foreground">No messages</p></div>
            ) : messages.map((msg) => (
              <div key={msg.id} className="p-3 rounded-lg bg-muted/30 mb-2">
