...
const AdminFinancePage = () => {
  ...
  const load = async () => {
    const [invRes, receiptRes, stuRes, clsRes, termRes, discRes, auditRes, tplRes] = await Promise.all([
      ...
      supabase.from("receipts").select("*, students(first_name, last_name, student_id), invoices(serial)").order("issued_at", { ascending: false }).limit(500),
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "receipts" }, () => void load())
      .on("postgres_changes", { event: "*", schema: "public", table: "invoice_lines" }, () => void load())
      .subscribe();
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <Tabs defaultValue="invoices">
          ...
          <TabsContent value="receipts" className="mt-4">
            <Card className="border-border/50">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-semibold">Issued receipts</CardTitle>
              </CardHeader>
              <CardContent className="p-0 overflow-x-auto">
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {receipts.map((receipt) => (
                      <TableRow key={receipt.id}>
                        <TableCell className="text-xs font-mono">{receipt.serial}</TableCell>
                        <TableCell className="text-sm">
                          {receipt.students?.first_name} {receipt.students?.last_name}
                          <span className="block text-[10px] text-muted-foreground font-mono">{receipt.students?.student_id}</span>
                        </TableCell>
                        <TableCell className="text-xs font-mono">{receipt.invoices?.serial || "—"}</TableCell>
                        <TableCell className="text-xs capitalize">{receipt.method}</TableCell>
                        <TableCell className="text-xs text-muted-foreground">{receipt.reference || "—"}</TableCell>
                        <TableCell className="text-sm text-right">{naira(receipt.amount)}</TableCell>
                    ...
                    {receipts.length === 0 && (
                      <TableRow><TableCell colSpan={7} className="text-center py-8 text-sm text-muted-foreground">No receipts have been issued.</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </CardContent>
