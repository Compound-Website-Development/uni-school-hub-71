...
import { ASSESSMENT_WEIGHTS, GRADE_BANDS, letterGradeForScore } from "@/lib/schoolConfig";
...
const AdminSettingsPage = () => {
  ...
  const gradingScale = GRADE_BANDS.map((band) => ({
    range: `${band.min}–${band.max}`,
    grade: letterGradeForScore(band.min),
    remark: band.remark,
  }));
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-xs text-muted-foreground mb-3">
                  CA ({ASSESSMENT_WEIGHTS.ca}) + Exam ({ASSESSMENT_WEIGHTS.exam}) = {ASSESSMENT_WEIGHTS.total}. This scale is applied automatically when grades are entered.
                </p>
                <Table>
                  <TableHeader>
                    <TableRow>
