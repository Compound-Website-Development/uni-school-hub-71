...
const AdminReportsPage = () => {
  ...
  useEffect(() => {
    loadReportContext().then(({ students: reportStudents }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      if (reportType === "students") {
        ...
        const rows = ((data as any[]) || []).map((t: any) => ({ employee_id: t.employee_id, first_name: t.first_name, last_name: t.last_name, email: t.email, phone: t.phone, department: t.department, status: t.status, hire_date: t.hire_date, created_at: t.created_at }));
        downloadCSV(rows, "staff_report.csv");
        ...
        <Card className="rounded-xl border-border/50 shadow-card">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              <FileText className="w-4 h-4" /> Official Report Card Viewer
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {reportStudentId ? (
              <CanonicalReportCard studentId={reportStudentId} requirePublished={false} />
            ) : (
              <p className="text-sm text-muted-foreground">
                Select a pupil to view the same report card used by the Staff Portal.
              </p>
            )}
          </CardContent>
        </Card>
