...
import { Search, Plus, Eye, Edit, UserX, UserCheck, GraduationCap, Upload, BadgeCheck, User, KeyRound, Loader2, Copy, Download } from "lucide-react";
...
const AdminStudentsPage = () => {
  ...
  /** Creates pupil logins (admission-number addresses) for the current class filter. */
  const provisionLogins = async () => {
    setProvisioning(true);
    const { data, error } = await supabase.functions.invoke("provision-student-accounts", {
      body: { class_id: classFilter === ALL ? null : classFilter },
    });
    ...
    setAccounts(list);
    const created = list.filter((a: any) => a.status === "created").length;
    toast.success(created ? `${created} pupil login(s) created` : "Every pupil in this view already has a login");
    fetchStudents();
  };

  const copyCredentials = () => {
    navigator.clipboard.writeText(
      (accounts || []).map((a) => `${a.name} — ${a.email} — ${a.password || "(existing password)"}`).join("\n"),
    );
    toast.success("Credentials copied");
  };

  const downloadCredentials = () => {
    const rows = [["Name", "Admission No", "Login email", "Password", "Class", "Status"]]
      .concat((accounts || []).map((a) => [a.name, a.admission_no || "", a.email || "", a.password || "", a.class_name || "", a.status]))
      .map((r) => r.map((v) => `"${String(v).replace(/"/g, '""')}"`).join(","))
      .join("\n");
    const url = URL.createObjectURL(new Blob([rows], { type: "text/csv" }));
    const link = document.createElement("a");
    link.href = url;
    link.download = "imagemakers-pupil-logins.csv";
    link.click();
    ...
    <AdminLayout title="Manage Students" showSearch>
      ...
      <Dialog open={!!accounts} onOpenChange={(v) => !v && setAccounts(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader><DialogTitle>Pupil logins</DialogTitle></DialogHeader>
          <p className="text-xs text-muted-foreground">
            Passwords are shown once only — copy or download them now, then hand each pupil their slip.
            Logins use the admission number, so no pupil email address is required.
          </p>
              ...
              <TableBody>
                {(accounts || []).map((a, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell className="text-xs font-mono break-all">{a.email || "—"}</TableCell>
                    <TableCell className="text-xs font-mono">{a.password || "—"}</TableCell>
                    <TableCell>
                      <Badge variant={a.status === "created" ? "default" : "secondary"} className="text-[10px]">{a.status}</Badge>
                    </TableCell>
                  </TableRow>
                ))}
                {!(accounts || []).length && (
                  <TableRow>
                    <TableCell colSpan={4} className="py-6 text-center text-sm text-muted-foreground">
                      Every active pupil in this view already has a login.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
          ...
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="outline" className="gap-1.5" onClick={copyCredentials}><Copy className="w-4 h-4" /> Copy all</Button>
            <Button size="sm" className="gap-1.5" onClick={downloadCredentials}><Download className="w-4 h-4" /> Download CSV</Button>
          </div>
        </DialogContent>
      </Dialog>
    </AdminLayout>
