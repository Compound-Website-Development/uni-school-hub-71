...
const TakeExam = () => {
  ...
  useEffect(() => {
    const fetchExam = async () => {
      ...
      const [{ data: examData }, { data: questionData }] = await Promise.all([
        ...
        (supabase as any).rpc("student_exam_questions", { _exam_id: examId }),
      ]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Persist the latest answers, then let the server mark the paper against
    // the answer key (students never receive `correct_index`).
    await supabase
      .from("exam_submissions")
      .update({ answers })
      .eq("id", submissionId);

    const { data, error } = await (supabase as any).rpc("submit_exam", { _submission_id: submissionId });
    if (error) {
    ...
    if (result?.total_points) {
      toast.success(`Exam submitted — you scored ${result.score} of ${result.total_points}.`);
    } else {
      toast.success("Exam submitted successfully!");
    }
    navigate("/student/exams");
  }, [isSubmitting, submissionId, answers, navigate]);
