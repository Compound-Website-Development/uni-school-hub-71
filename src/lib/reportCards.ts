...
/** Read a pupil's stored report card for a term from grades and term_results. */
export const loadReportCard = async (
  ...
  const [{ data: grades }, { data: result }, { data: classRow }, { data: settings }] =
    await Promise.all([
      supabase
        .from("grades")
      ...
      student.class_id
        ? supabase
            .from("classes")
            .select("teachers:class_teacher_id (first_name, last_name)")
            .eq("id", student.class_id)
            .maybeSingle()
        : Promise.resolve({ data: null } as any),
      supabase.from("school_settings").select("key, value").eq("key", "head_teacher_name").maybeSingle(),
    ]);

  const classTeacher = (classRow as any)?.teachers;
  const classTeacherName = classTeacher ? `${classTeacher.first_name} ${classTeacher.last_name}` : "";
  const headTeacherName = (settings as any)?.value || "";
  ...
  const rows = scored.filter((s) => s.subject);
  ...
  return {
    ...
    classTeacherName,
    headTeacherName,
    timesOpened: r?.times_opened != null ? String(r.times_opened) : "",
    timesPresent: r?.times_present != null ? String(r.times_present) : "",
    nextTermBegins: r?.next_term_begins || "",
    gpa: r?.gpa != null ? Number(r.gpa).toFixed(2) : "",
  };
};
...
export const saveReportCard = async (
  ...
  if (scored.length) {
    const gradeRows = scored.map((s) => {
      const subjectId = byName.get(s.subject.trim().toLowerCase());
      ...
      return {
        ...
        continuous_assessment: s.ca,
        exam_score: s.exam,
        status: publish ? "approved" : "draft",
      };
    });
    if (gradeRows.some((row) => row === null)) {
      return { error: "Could not match one or more report-card subjects." };
    }

    const { error } = await supabase.from("grades").upsert(
      gradeRows.filter((row): row is NonNullable<typeof row> => row !== null),
